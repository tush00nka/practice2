...
#let template(
  ...
  show enum: it => context {
    ...
    enum-counter.update(n => 0) // Start from 0 so first becomes 1

    for item in it.children {
      block({
        enum-counter.update(n => n + 1)

        context {
          h(it.indent)
          // Get the current counter value as a single integer
          let current-value = enum-counter.get().at(0)
          numbering(it.numbering, current-value)
          h(it.body-indent)
        }
        item.body
        parbreak()
